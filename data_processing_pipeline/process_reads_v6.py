...
with utils.measureTime('CDS gtf models are loaded'):
    with open('all_protein_coding.v38.75.txt') as protlist:
...
with utils.measureTime('Finished parsing {}'.format(','.join(args.files))):
...
#                        sys.stderr.write('{}|{} is not in the DB\n'.format(gene_id, tr_id)
                location= 'db_problem'
            genes_by_read[gene_id].add((tr_id, location))
                #if track_multi:
